...
public:
    ...
    BigInteger& operator += (const BigInteger& b) {
        ...
        size_t tmp = 0;
        size_t n = number_.size() > b.number_.size() ? number_.size() : b.number_.size();
    ...
    BigInteger& operator -= (const BigInteger& b) {
        ...
        for (size_t i = 0; i < b.number_.size(); ++i) {
            tmp += number_[i] - b.number_[i];
            if (tmp < 0) {
                number_[i] = (b.base_ + tmp);
                tmp = -1;
            } else {
                number_[i] = (tmp % b.base_);
                tmp /= b.base_;
            }
        }
        for (size_t i = b.number_.size(); i < number_.size(); ++i) {
            tmp += number_[i];
            if (tmp < 0) {
                number_[i] = (base_ + tmp);
                tmp = -1;
        ...
        while (number_[number_.size() - 1] == 0 && number_.size() > 1) {
    ...
    friend BigInteger operator * (const BigInteger& a, const BigInteger& b) {
        ...
        if (n == 1) {
            return BigInteger(x.toIntAbs() * y.toIntAbs(), a.sign_ == b.sign_);
        }
        size_t n_fix = n;
    ...
    friend BigInteger operator / (const BigInteger& a, const BigInteger& b) {
        ...
        BigInteger left(makeBasePow(range_helper - 1));
        BigInteger right(makeBasePow(range_helper + 1));
    ...
    /* double cast operator BE CAREFUL when you use it */
...
private:
    /* make Base pow string */
    static std::string makeBasePow(ssize_t pow) {
        if (pow <= 0) {
            return "0";
        }
        std::string t = "1";
    ...
    /* get new BigInteger with old from numbers range */
...
int main() {
    ...
    BigInteger f("505");
    BigInteger p("-25000");
    BigInteger o("10000000000");
    BigInteger i("-10000000000");
    BigInteger r("343");

    assert(a + b == BigInteger("14000"));
    assert(a + c == BigInteger("9004"));
    assert(a + d == BigInteger("8995"));
    assert(c + d == BigInteger("-1"));
    assert(c + p == BigInteger("-24996"));
    assert(d + c == BigInteger("-1"));
    ...
    assert(c - b == BigInteger("-4996"));
    assert(f - p == BigInteger("25505"));
    assert(d - c == BigInteger("-9"));
    assert(o - i == BigInteger("20000000000"));
    assert(r - i == BigInteger("10000000343"));
